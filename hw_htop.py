...
class Cpu(GetInfo):
    ...
    def __str__(self):
        cputime_temp = ('| {user_time:^10} | {system_time:^11} '
                        '| {time_nothin_do:^10} | {nice_time:^9} |')

        cpustats_temp = ('| {sum_switch:^10} | {sum_iter:^11} '
                        '| {sun_softiter:^12} | {sum_call:^9} |')

        cpupercent_temp = ('| {use_cpu1:^5} | {use_cpu2:^5} '
        ...
        str10 = (
                ...
                ' CPU{4}-%| CPU{5}-%|'
                ' CPU{6}-%| CPU{7}-%|\n'.format(*range(1,
                    len(psutil.cpu_percent(1,True)) + 1)
                    )
                    )
...
class VirtualMemory(GetInfo):
    ...
    def __str__(self):
        ...
        str_mem2 = (
        '| {:^16} | {:^16} | {:^16} |'.format('total memory GB',
                                               'available GB',
                                               'used memory GB')
                                                )
