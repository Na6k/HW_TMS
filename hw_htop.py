...
class Cpu(GetInfo):
    def get_info(self):  # It's function return information about your CPU
        ...
        self.res_cpu.update(
            use_cpu1=date_per[0],
            use_cpu2=date_per[1],  # The numbers CPU(№)
            use_cpu3=date_per[2],
            use_cpu4=date_per[3],
            use_cpu5=date_per[4],
            use_cpu6=date_per[5],
    ...
    def __str__(self):
        cputime_temp = (
            "| {user_time:^10} | {system_time:^11} " "| {time_nothin_do:^10} | {nice_time:^9} |"
        )

        cpustats_temp = (
            "| {sum_switch:^10} | {sum_iter:^11} " "| {sun_softiter:^12} | {sum_call:^9} |"
        )
        ...
        str2 = "| {:^10} | {:^11} |" " {:^10} | {:^9} |".format(
            "user time", "system time", "nothing do", "nice time"
        )
        str3 = cputime_temp.format(**self.res_cpu)
        str4 = len(cputime_temp.format(**self.res_cpu)) * "-"
        str5 = "{:^55}".format("<<Information about SWITCH>>")
        str6 = "| {:^10} | {:^11} |" " {:^12} | {:^9} |".format(
            "sum switch", "sum iter", "sum softiter", "sum call"
        )
        ...
        str10 = (
            ...
            " CPU{2}-%| CPU{3}-%|"
            " CPU{4}-%| CPU{5}-%|"
            " CPU{6}-%| CPU{7}-%|\n".format(*range(1, len(psutil.cpu_percent(1, True)) + 1))
        )
...
class VirtualMemory(GetInfo):
    def get_info(
        self,
    ):  # It's function return statistic about used system(virtual) memory
        virtual_mem = {}
    ...
    def __str__(self):
        memory_templage = (
            "| {total_memory:^16.0f} " "| {available_memory:^16.1f} " "| {used_memory:^16.1f} |"
        )
        str_mem1 = "{:^58}".format("Information about memory RAM")
        str_mem2 = "| {:^16} | {:^16} | {:^16} |".format(
            "total memory GB", "available GB", "used memory GB"
        )
...
class Disk(GetInfo):
    ...
    def __str__(self):
        ...
        str2_disk = "| {:^15} | {:^15} " "| {:^15} | {:^10} |".format(
            "total disk GB", "used disk GB", "free disk GB", "usedisk(%)"
        )
...
class NetworkInfo(GetInfo):
    ...
    def __str__(self):
        ...
        str2_network = "| {} | {} | {} |".format("send butes", "received bytes", "ERROR")
...
class UserInfo(GetInfo):
    ...
    def __str__(self):
        ...
        str2_user = (
            "| {:^10} " "| {:^10} " "| {:^10} |".format("user name", "type cmd", "start time")
        )
